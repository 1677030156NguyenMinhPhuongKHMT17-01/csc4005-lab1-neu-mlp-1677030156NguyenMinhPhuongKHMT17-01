...
|----------|-----------|----|--------------|---------|---------------|--------------|----------|
| baseline_adamw | AdamW | 0.001 | 0.0001 | 0.3 | 1.499 | 41.85% | 38.15% |
| run_b_sgd | SGD | 0.01 | 0.0 | 0.3 | **1.443** | **48.89%** | **47.41%** |
| run_c_strong_reg | AdamW | 0.0005 | 0.001 | 0.5 | 1.628 | 32.96% | 34.44% |

*Nhận xét từ biểu đồ learning curve:*
- Lượt chạy **SGD (run_b_sgd)** đạt hiệu suất cao nhất. Với Learning Rate được chỉnh lên 0.01, tốc độ giảm loss và tăng độ chính xác vượt trội hơn hẳn so với AdamW baseline.
- Lượt chạy **Baseline (baseline_adamw)** học khá nhanh ở những epoch rọi nhưng bắt đầu dao động lớn về độ chính xác xác cũng như loss.
- Lượt chạy **Strong Regularization (run_c_strong_reg)** (đường curve cao nhất ở loss) do bị kìm kẹp bởi Dropout cao (0.5), Weight Decay lớn (0.001) và LR thấp (0.0005) nên tiến trình học bị chậm đi rõ rệt, kết quả accuracy xếp chót trong 3 cấu hình sau 20 epoch.
...
## 5. Kết luận
- **Best Config:** Lựa chọn run_b_sgd (Optimizer: SGD, LR: 0.01, Weight Decay: 0.0, Dropout: 0.3).
